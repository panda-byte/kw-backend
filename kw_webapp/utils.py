...
def prefetch_synonyms_for_reviews(reviews):
    # Fetches synonyms for reviews/lessons in bulk and assigns
    # the found synonyms to each `Vocabulary._prefetched_synonyms`
    # property. You should prefetch "vocabulary__meanings" and
    # "vocabulary__meanings__meaning" of the previews before
    # calling this function to optimize DB queries.
    all_synonymous_meaning_ids = [
        mapping.meaning_id
        for review in reviews
        for mapping in review.vocabulary.meanings.all()
    ]

    all_synonymous_vocabulary = list(
        Vocabulary.objects.filter(
            meanings__meaning_id__in=all_synonymous_meaning_ids
        ).prefetch_related(
            "readings",
            "meanings",
            "meanings__meaning"
    ...
    for review in reviews:
        synonymous_meaning_ids = [
            mapping.meaning_id
            for review in reviews
            for mapping in review.vocabulary.meanings.all()
        ]

        prefetched_synonyms = set(
            vocabulary
            for vocabulary in all_synonymous_vocabulary
            if (
                    vocabulary != review.vocabulary
                    and any(
                mapping.meaning_id in synonymous_meaning_ids
                for mapping in vocabulary.meanings.all()
            )
            )
        )

        if prefetched_synonyms:
            review.vocabulary._prefetched_synonyms = list(prefetched_synonyms)
